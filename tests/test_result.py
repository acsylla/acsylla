...
class TestResult:
    ...
    async def test_result_one_row(self, session, insert_statement, select_statement, id_generation):
        ...
        assert row.column_value("value") == value
    ...
    async def test_result_invalid_column_name(self, session, id_generation):
        ...
        with pytest.raises(ColumnNotFound):
            row.column_value("invalid_column_name")
    ...
    async def test_result_multiple_rows(self, session, insert_statement, select_filter_statement, id_generation):
        ...
        ids_returned = [row.column_value("id") for row in result.all()]
        values_returned = [row.column_value("value") for row in result.all()]
    ...
    @pytest.mark.parametrize("type_", ["prepared", "none_prepared"])
    async def test_result_paging_reuse_statement(
        self, session, id_generation, type_, page_size, pages_fetched_expected
    ):
        total_rows = 100
        value = 33
        insert_statement = "INSERT INTO test (id, value) values(?, ?)"
        ...
        statement = await self._build_statement(session, type_, select_statement, 2)
        statement.bind_list([ids[0], ids[-1]])
